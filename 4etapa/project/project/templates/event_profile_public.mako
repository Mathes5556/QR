...
//only public..
  heslo_zamestnanca = '${str(employee.lock_for_public)}';
  $('#main_content').hide();

  $( "#verify_password" ).click(function() {
    if(heslo_zamestnanca == $('#password_to_system').val()){
      $('#main_content').show();
      $('#zadaj_heslo').hide();
    }
    else{
      alert('Bad password autentification');
    }
});
...
<div id="zadaj_heslo">
    <div class="form-group">
        ...
        <input  id ="password_to_system"name="password" class="form-control"  placeholder="password">
    </div>
    <button type="button" class="btn btn-success" id="verify_password">Verify</button>
</div>
