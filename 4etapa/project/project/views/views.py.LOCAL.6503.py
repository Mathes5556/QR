...
def topky_parser(request):
    ...
    for blog in blogs:
        ...
        if kedy == 'dnes':
            print('ok')
            load_content_from_blog_sme(odkaz)
        else:
            break

@view_config(route_name='topky_parser', renderer='project:templates/home.mako')
def topky_parser(request):
    #TODO: prejst cez vsetky kategorie, sparsovat pre kazdu
    # kategorie = {
    #             '10': 'Domace',
    #             '11': 'Zahranicne',
    #             '15': 'Prominenti',
    ...
    for i in range(1):
        ...
        for blog in blogs:
            ...
            kedy = blog.p.string
            print('- Parsujem ' + odkaz)
            load_content_from_topky(odkaz, request)
            print('ok')
        page_number += 1
        
    return{'text': 'some random text'}

def load_content_from_topky(source, request):
    html_source_url = source
    htmltxt = req.urlopen(html_source_url)
    soup = BeautifulSoup(htmltxt)
    ...
    article = soup.body.find('div', attrs={'id': 'article_body'}).findAll('p')
    for paragraph in article:
        result += paragraph.text
    
    #spracuj_data_do_db(title, result, request)



# TODO: funkcia ktora zisti datum na zaklade casu pre topky.sk
# def topky_find_article_time(kedy):
#     now = time.now()

#     pred = str(kedy).split(' ')
#     if pred[0].find('pred'):
#         if pred[3].find('minútami'):
#             minutes = int(pred[2])
#             seconds = minutes * 60
#             return 
@view_config(route_name='spracuj', renderer='project:templates/home.mako')
def spracuj(request):
    today = str(datetime.date.today())+" 00:00:00.000000" 

    ar=request.db_session.query(Article).filter_by(date=today).first()
    print(ar)
    return {'text': 'mihu', 'vysledok' : '5'}